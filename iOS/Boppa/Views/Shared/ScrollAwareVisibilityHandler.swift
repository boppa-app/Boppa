    ...
    func handleScrollChange(oldInfo: ScrollInfo, newInfo: ScrollInfo, isSearchFieldFocused: Bool) {
        ...
        // Scrolling up - use velocity + distance-based detection
            ...
            if velocity > velocityThreshold || spacerAboutToShow {
                ...
                if self.accumulatedScrollUp > scrollUpThreshold || spacerAboutToShow,
                   !self.isHeaderVisible
                {
                    // Dynamically calculate animation duration based on how quickly the scroll will
                    // reach the top. This ensures the search bar finishes appearing before the
                    // spacer is fully exposed
