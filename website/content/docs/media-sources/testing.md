...
There is no in-app "reload configuration" action separate from version-based updates. You can try local edits by picking the `.yaml` file directly off your device. A media source added this way has no `configUrl`, so deleting and adding the file after each edit is the only way to update it.
